...
public:
                      ...
                      NodeListIt rhs)
    {
        if (lhs->isPending())
        {
            schoco_details::list::steal(_blockeds, _pendings, lhs);
        }
    ...
    void detach(EdgeListIt edgeIt)
    {
        ...
        {
            schoco_details::list::steal(_pendings, _blockeds, edgeIt->_dependerNode);
        }
...
struct TTraits
{
    ...
    using EdgeList = schoco_details::list::TMake<Edge, SIZE>;
    using NodeList = schoco_details::list::TMake<Node, SIZE>;

    using NodeListIt = typename NodeList::iterator;
    using EdgeListIt = typename EdgeList::iterator;

    using EdgeItPool = typename schoco_details::list::TMake<EdgeListIt, SIZE>::CellPool;
